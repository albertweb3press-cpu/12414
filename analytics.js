/**
 * Аналитика для лабораторного исследования (с согласия пользователя)
 * Вставь в index.html перед </body>: <script src="https://ТВОЙ-VERCEL-ДОМЕН/analytics.js"></script>
 */
(function () {
  const ANALYTICS_URL = 'https://YOUR-VERCEL-APP.vercel.app/api/visit'; // <-- замени на свой домен Vercel
  ...
  // отправляем 1 раз за сессию
  if (!sessionStorage.getItem('logged_visit')) {
    logVisit();
    sessionStorage.setItem('logged_visit', '1');
  }

  // для события подключения кошелька: window.logWalletConnect(['Phantom'])
